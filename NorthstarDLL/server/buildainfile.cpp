...
void DumpAINInfo(CAI_Network* aiNetwork)
{
	...
	for (int i = 0; i < aiNetwork->nodecount; i++)
	{
		...
		spdlog::info(
			"writing node {} from {} to {:x}", aiNetwork->nodes[i]->index, (void*)aiNetwork->nodes[i], (__int64)writeStream.tellp());
	...
	for (int i = 0; i < aiNetwork->nodecount; i++)
	{
		for (int j = 0; j < aiNetwork->nodes[i]->linkcount; j++)
		{
			...
			spdlog::info("writing link {} => {} to {:x}", diskLink.srcId, diskLink.destId, (__int64)writeStream.tellp());
	...
	spdlog::info("writing {:x} bytes for unknown block at {:x}", aiNetwork->nodecount * sizeof(uint32_t), (__int64)writeStream.tellp());
	...
	spdlog::info("writing {} traversal nodes at {:x}...", 0, (__int64)writeStream.tellp());
	...
	spdlog::info("writing {} bytes for unknown hull block at {:x}", MAX_HULLS * 8, (__int64)writeStream.tellp());
	...
	spdlog::info("writing {} unknown node structs at {:x}", *pUnkStruct0Count, (__int64)writeStream.tellp());
	writeStream.write((char*)pUnkStruct0Count, sizeof(*pUnkStruct0Count));
	for (int i = 0; i < *pUnkStruct0Count; i++)
	{
		spdlog::info("writing unknown node struct {} at {:x}", i, (__int64)writeStream.tellp());
	...
	spdlog::info("writing {} unknown link structs at {:x}", *pUnkLinkStruct1Count, (__int64)writeStream.tellp());
	...
	for (int i = 0; i < *pUnkLinkStruct1Count; i++)
	{
		...
		spdlog::info("writing unknown link struct {} at {:x}", i, (__int64) writeStream.tellp());
	...
	spdlog::info("writing {} script nodes at {:x}", aiNetwork->scriptnodecount, (__int64)writeStream.tellp());
	...
	{
		// disk and memory structs are literally identical here so just directly write
		spdlog::info("writing script node {} at {:x}", i, (__int64)writeStream.tellp());
		writeStream.write((char*)&aiNetwork->scriptnodes[i], sizeof(aiNetwork->scriptnodes[i]));
	}

	spdlog::info("writing {} hints at {:x}", aiNetwork->hintcount, (__int64)writeStream.tellp());
	writeStream.write((char*)&aiNetwork->hintcount, sizeof(aiNetwork->hintcount));
	for (int i = 0; i < aiNetwork->hintcount; i++)
	{
		spdlog::info("writing hint data {} at {:x}", i, (__int64)writeStream.tellp());
