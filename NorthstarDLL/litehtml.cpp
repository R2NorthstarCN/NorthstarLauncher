...
{
	HTMLDocumentContainer(StudioApp& app) : m_app(app), m_images(app.getWorldEditor()->getAllocator()) {}
	...
	void draw_background(litehtml::uint_ptr hdc, const litehtml::background_paint& bg) override
	{
		...
		case litehtml::background_repeat_no_repeat:
			win->DrawList->AddImage(img.texture, a, b);
			break;
	...
	std::shared_ptr<litehtml::element> create_element(
		const litehtml::tchar_t* tag_name, const litehtml::string_map& attributes, const std::shared_ptr<litehtml::document>& doc) override
	{
		return nullptr;
	}
...
{
	HTMLPlugin(StudioApp& app) : m_app(app), m_html_container(app) {}
	...
	bool loadFile(const char* path, Array<u8>* data)
	{
		...
		FS::OsFile file;
		IAllocator& allocator = m_app.getWorldEditor()->getAllocator();
		if (!file.open(path, FS::Mode::OPEN_AND_READ, allocator))
			return false;
